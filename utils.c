...
/**
 * Get the difference between two angles. Will always be between -180 and +180
 * degrees.
 * @param angle1
 * The first angle
 * @param angle2
...
float utils_throttle_curve(float val, float curve_acc, float curve_brake, int mode)
{
	...
	{	 // Natural
		...
		else
		{
			...
			{
				ret = 1.0 -
					  ((expf(curve * (1.0 - val_a)) - 1.0) / (expf(curve) - 1.0));
			}
			else
			{
...
/**
 * A system locking function with a counter. For every lock, a corresponding
 * unlock must exist to unlock the system. That means, if lock is called five
 * times, unlock has to be called five times as well. Note that chSysLock and
 * chSysLockFromIsr are the same for this port.
 */
...
/**
 * A system unlocking function with a counter. For every lock, a corresponding
 * unlock must exist to unlock the system. That means, if lock is called five
 * times, unlock has to be called five times as well. Note that chSysUnlock and
 * chSysUnlockFromIsr are the same for this port.
 */
...
void utils_fft32_bin0(float* real_in, float* real, float* imag)
{
	...
	*imag = 0.0;
	...
	for (int i = 0; i < 32; i++) { *real += real_in[i]; }
...
void utils_fft16_bin0(float* real_in, float* real, float* imag)
{
	...
	*imag = 0.0;
	...
	for (int i = 0; i < 16; i++) { *real += real_in[i]; }
...
void utils_fft8_bin0(float* real_in, float* real, float* imag)
{
	...
	*imag = 0.0;
	...
	for (int i = 0; i < 8; i++) { *real += real_in[i]; }
...
float utils_batt_lion_norm_v_to_capacity(float norm_v)
{
	...
	const float li_p[] = {-2.979767f, 5.487810f, -3.501286f, 1.675683f, 0.317147f};
	utils_truncate_number(&norm_v, 0.0f, 1.0f);
	float v2	   = norm_v * norm_v;
	float v3	   = v2 * norm_v;
	float v4	   = v3 * norm_v;
	float v5	   = v4 * norm_v;
	float capacity = li_p[0] * v5 + li_p[1] * v4 + li_p[2] * v3 + li_p[3] * v2 +
					 li_p[4] * norm_v;
	return capacity;
}
...
const float utils_tab_sin_32_1[] = {0.000000,
	0.195090,
	0.382683,
	0.555570,
	0.707107,
...
const float utils_tab_sin_32_2[] = {0.000000,
	0.382683,
	0.707107,
	0.923880,
	1.000000,
...
const float utils_tab_cos_32_1[] = {1.000000,
	0.980785,
	0.923880,
	0.831470,
	0.707107,
...
const float utils_tab_cos_32_2[] = {1.000000,
	0.923880,
	0.707107,
	0.382683,
	0.000000,
