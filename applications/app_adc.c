...
static THD_FUNCTION(adc_thread, arg)
{
	...
	for (;;)
	{
		...
		case ADC_CTRL_TYPE_PID_REV_CENTER:
			...
			{
				pwr = utils_map(
					pwr, config.voltage_start,
					config.voltage_center, 0.0, 0.5);
			}
			else
			{
				pwr = utils_map(
					pwr, config.voltage_center,
		...
		float
			ramp_time = fabsf(pwr) > fabsf(pwr_ramp) ? config.ramp_time_pos : config.ramp_time_neg;
...
//		}
		...
		{
			const float
				ramp_step = (float)ST2MS(chVTTimeElapsedSinceX(last_time)) / (ramp_time * 1000.0);
					...
					for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
					{
						...
						if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE)
						{
							if (config.tc)
							{
								...
								current_out =
									utils_map(diff, 0.0, config.tc_max_diff, current_rel, 0.0);
							}
