...
#define SINCOS_MIN_AMPLITUDE                                                   \
	1.0	   // sqrt(sin^2 + cos^2) has to be larger than this
#define SINCOS_MAX_AMPLITUDE                                                   \
	1.65	// sqrt(sin^2 + cos^2) has to be smaller than this

#if (AS5047_USE_HW_SPI_PINS) || (MT6816_USE_HW_SPI_PINS) ||                    \
	(AD2S1205_USE_HW_SPI_PINS)
#ifdef HW_SPI_DEV
...
uint32_t encoder_spi_get_error_cnt(void) { return spi_error_cnt; }

uint32_t encoder_spi_get_val(void) { return spi_val; }

float encoder_spi_get_error_rate(void) { return spi_error_rate; }
...
void encoder_ts57n8501_reset_errors(void) { ts5700n8501_reset_errors = true; }
...
void encoder_deinit(void)
{
	...
	if (mode == ENCODER_MODE_TS5700N8501)
	{
		...
		while (ts5700n8501_is_running) { chThdSleepMilliseconds(1); }
...
void encoder_init_abi(uint32_t counts)
{
	...
	palSetPadMode(HW_HALL_ENC_GPIO1,
		HW_HALL_ENC_PIN1,
		PAL_MODE_ALTERNATE(HW_ENC_TIM_AF));
	palSetPadMode(HW_HALL_ENC_GPIO2,
		HW_HALL_ENC_PIN2,
		PAL_MODE_ALTERNATE(HW_ENC_TIM_AF));
	...
	TIM_EncoderInterfaceConfig(HW_ENC_TIM,
...
void encoder_init_as5047p_spi(void)
{
	...
	palSetPadMode(SPI_SW_SCK_GPIO,
		SPI_SW_SCK_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	palSetPadMode(SPI_SW_CS_GPIO,
		SPI_SW_CS_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);

	// Set MOSI to 1
#if (AS5047_USE_HW_SPI_PINS || AD2S1205_USE_HW_SPI_PINS)
	palSetPadMode(SPI_SW_MOSI_GPIO,
		SPI_SW_MOSI_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
void encoder_init_mt6816_spi(void)
{
	...
	palSetPadMode(SPI_SW_MISO_GPIO,
		SPI_SW_MISO_PIN,
		PAL_MODE_ALTERNATE(6) | PAL_STM32_OSPEED_HIGHEST);
	palSetPadMode(SPI_SW_CS_GPIO,
		SPI_SW_CS_PIN,
...
void encoder_init_ad2s1205_spi(void)
{
	...
	palSetPadMode(AD2S1205_SAMPLE_GPIO,
		AD2S1205_SAMPLE_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	...
	palSetPadMode(AD2S1205_RDVEL_GPIO,
		AD2S1205_RDVEL_PIN,
		PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
	...
	TIM_TimeBaseStructure.TIM_Period =
		((168000000 / 2 / AD2S1205_SAMPLE_RATE_HZ) - 1);
	TIM_TimeBaseStructure.TIM_ClockDivision		= 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(HW_ENC_TIM, &TIM_TimeBaseStructure);
...
void encoder_init_sincos(float s_gain,
	float					   s_offset,
	float					   c_gain,
	float					   c_offset,
...
void encoder_init_ts5700n8501(void)
{
	...
	chThdCreateStatic(ts5700n8501_thread_wa,
		sizeof(ts5700n8501_thread_wa),
		NORMALPRIO - 10,
		ts5700n8501_thread,
		NULL);
}

bool encoder_is_configured(void) { return mode != ENCODER_MODE_NONE; }
...
float encoder_read_deg(void)
{
	...
	case ENCODER_MODE_ABI:
		angle = ((float)HW_ENC_TIM->CNT * 360.0f) / (float)enc_counts;
		break;
	...
	case ENCODER_MODE_SINCOS: {
		...
		{
			// signals vector outside of the valid area. Increase error count
			// and discard measurement
			++sincos_signal_above_max_error_cnt;
			UTILS_LP_FAST(sincos_signal_above_max_error_rate,
				1.0,
				1. / SINCOS_SAMPLE_RATE_HZ);
			angle = last_enc_angle;
		}
		else
		{
			if (module < SQ(SINCOS_MIN_AMPLITUDE))
			{
				...
				UTILS_LP_FAST(sincos_signal_low_error_rate,
					1.0,
					1. / SINCOS_SAMPLE_RATE_HZ);
				angle = last_enc_angle;
			}
...
void encoder_tim_isr(void)
{
	...
	if (mode == ENCODER_MODE_AS5047P_SPI)
	{
		...
		if (spi_check_parity(pos))
		{
			...
			UTILS_LP_FAST(spi_error_rate, 0.0f, 1.0f / AS5047_SAMPLE_RATE_HZ);
		}
		else
		{
			++spi_error_cnt;
			UTILS_LP_FAST(spi_error_rate, 1.0f, 1.0f / AS5047_SAMPLE_RATE_HZ);
		}
	}
	...
	if (mode == ENCODER_MODE_MT6816_SPI)
	{
		...
		if (spi_check_parity(pos))
		{
			if (pos & MT6816_NO_MAGNET_ERROR_MASK)
			{
				...
				UTILS_LP_FAST(encoder_no_magnet_error_rate,
					1.0,
					1. / MT6816_SAMPLE_RATE_HZ);
			}
			else
			{
				pos			   = pos >> 2;
	...
	if (mode == RESOLVER_MODE_AD2S1205)
	{
		...
		if ((RDVEL != 0))
		{
			...
			if (LOT)
			{
				...
				UTILS_LP_FAST(resolver_loss_of_tracking_error_rate,
					1.0,
			...
			if (DOS)
			{
				...
				UTILS_LP_FAST(resolver_degradation_of_signal_error_rate,
					1.0,
					1. / AD2S1205_SAMPLE_RATE_HZ);
			}
			else
			...
			if (LOS)
			{
				...
				UTILS_LP_FAST(resolver_loss_of_signal_error_rate,
					1.0,
					1. / AD2S1205_SAMPLE_RATE_HZ);
			}
			else
 ...
 */
bool encoder_index_found(void) { return index_found; }
...
static void spi_begin(void) { palClearPad(SPI_SW_CS_GPIO, SPI_SW_CS_PIN); }

static void spi_end(void) { palSetPad(SPI_SW_CS_GPIO, SPI_SW_CS_PIN); }
...
/*
 * It is important to switch to receive mode immediately after sending the
 * readout command, as the TS5700N8501 starts sending the reply after 3
 * microseconds. Therefore use software UART on TX so that the enable signal can
 * be controlled manually. This function runs while the system is locked, but it
 * should finish fast enough to not cause problems for other things due to the
 * high baud rate.
 */
static void TS5700N8501_send_byte(uint8_t b)
{
...
static THD_FUNCTION(ts5700n8501_thread, arg)
{
	...
	for (;;)
	{
		...
		uint8_t crc = 0;
		for (int i = 0; i < (reply_ind - 1); i++) { crc = (reply[i] ^ crc); }
