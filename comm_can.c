...
void comm_can_init(void)
{
	...
	palSetPadMode(HW_CANRX_PORT,
				  ...
				  PAL_MODE_ALTERNATE(HW_CAN_GPIO_AF) |
					  PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_OSPEED_MID1);
 ...
 *
 * The callback should return true if the frame was used by the application, false
 * otherwise. if the frame was used, no further processing will be done here.
 *
 * @param p_func
...
void comm_can_send_buffer(uint8_t controller_id, uint8_t* data, unsigned int len, uint8_t send)
{
	...
	if (len <= 6)
	{
		...
		ind += len;
		comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PROCESS_SHORT_BUFFER
													   << 8),
									  send_buffer,
									  ind,
									  true);
	...
	else
	{
		...
		for (unsigned int i = 0; i < len; i += 7)
		{
			...
			comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_FILL_RX_BUFFER
														   << 8),
										  send_buffer,
										  send_len + 1,
										  true);
		...
		for (unsigned int i = end_a; i < len; i += 6)
		{
			...
			comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_FILL_RX_BUFFER_LONG
														   << 8),
										  send_buffer,
										  send_len + 2,
										  true);
		...
		comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PROCESS_RX_BUFFER
													   << 8),
									  send_buffer,
									  ind++,
									  true);
...
void comm_can_set_rpm(uint8_t controller_id, float rpm)
{
	...
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_RPM << 8),
								  buffer,
								  send_index,
								  true);
}
...
void comm_can_set_pos(uint8_t controller_id, float pos)
{
	...
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_POS << 8),
								  buffer,
								  send_index,
								  true);
}
...
void comm_can_set_current_brake_rel(uint8_t controller_id, float current_rel)
{
	...
	uint8_t buffer[4];
	buffer_append_float32(buffer, current_rel, 1e5, &send_index);
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE_REL
												   << 8),
								  buffer,
								  send_index,
								  true);
...
void comm_can_set_handbrake(uint8_t controller_id, float current)
{
	...
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE
												   << 8),
								  buffer,
								  send_index,
								  true);
...
void comm_can_set_handbrake_rel(uint8_t controller_id, float current_rel)
{
	...
	uint8_t buffer[4];
	buffer_append_float32(buffer, current_rel, 1e5, &send_index);
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE_REL
												   << 8),
								  buffer,
								  send_index,
								  true);
...
bool comm_can_ping(uint8_t controller_id, HW_TYPE* hw_type)
{
	...
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PING << 8),
								  buffer,
								  1,
								  true);
 ...
 */
void comm_can_detect_apply_all_foc(uint8_t controller_id,
								   ...
								   float max_power_loss)
{
	...
	comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC
												   << 8),
								  buffer,
								  send_index,
								  true);
...
void comm_can_conf_current_limits(uint8_t controller_id, bool store, float min, float max)
{
	...
	comm_can_transmit_eid_replace(
		controller_id | ((uint32_t)(store ? CAN_PACKET_CONF_STORE_CURRENT_LIMITS :
...
void comm_can_conf_current_limits_in(uint8_t controller_id, bool store, float min, float max)
{
	...
	buffer_append_float32(buffer, max, 1e3, &send_index);
	comm_can_transmit_eid_replace(
		controller_id | ((uint32_t)(store ? CAN_PACKET_CONF_STORE_CURRENT_LIMITS_IN :
							 ...
							 float foc_sl_erpm)
{
	...
	comm_can_transmit_eid_replace(
		controller_id |
			((uint32_t)(store ? CAN_PACKET_CONF_STORE_FOC_ERPMS : CAN_PACKET_CONF_FOC_ERPMS)
			 << 8),
		buffer,
		send_index,
		true);
...
int comm_can_detect_all_foc_res_size(void) { return detect_all_foc_res_index; }

void comm_can_detect_all_foc_res_clear(void) { detect_all_foc_res_index = 0; }
...
void comm_can_io_board_set_output_digital(int id, int channel, bool on)
{
	...
	comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_DIGITAL
										<< 8),
								  buffer,
								  send_index,
								  true);
...
void comm_can_io_board_set_output_pwm(int id, int channel, float duty)
{
	...
	comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_PWM
										<< 8),
								  buffer,
								  send_index,
								  true);
...
_Noreturn static THD_FUNCTION(cancom_process_thread, arg)
{
	(void)arg;
...
static void decode_msg(uint32_t eid, uint8_t* data8, int len, bool is_replaced)
{
	...
	if (id == 255 || id == id1 || id == id2)
	{
		...
		case CAN_PACKET_PING: {
			uint8_t buffer[2];
			buffer[0] = is_replaced ? utils_second_motor_id() :
										app_get_configuration()->controller_id;
			buffer[1] = HW_TYPE_VESC;
			comm_can_transmit_eid_replace(data8[0] | ((uint32_t)CAN_PACKET_PONG << 8),
										  buffer,
										  2,
										  true);
		}
		...
		case CAN_PACKET_DETECT_APPLY_ALL_FOC: {
			...
			int8_t buffer[1];
			buffer[0] = res;
			comm_can_transmit_eid_replace(data8[0] | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC_RES
													  << 8),
										  (uint8_t*)buffer,
										  1,
										  true);
		...
		case CAN_PACKET_POLL_TS5700N8501_STATUS: {
			comm_can_transmit_eid_replace(app_get_configuration()->controller_id |
											  ((uint32_t)CAN_PACKET_POLL_TS5700N8501_STATUS
											   << 8),
										  encoder_ts5700n8501_get_raw_status(),
										  8,
										  true);
		}
		break;
...
static void send_status4(uint8_t id, bool replace)
{
	...
	uint8_t buffer[8];
	buffer_append_int16(buffer,
						(int16_t)(mc_interface_temp_fet_filtered() * 1e1),
						&send_index);
	buffer_append_int16(buffer,
	...
	buffer_append_int16(buffer,
						...
						&send_index);
	buffer_append_int16(buffer,
						(int16_t)(mc_interface_get_pid_pos_now() * 50.0),
						&send_index);
