...
void observer_update(float v_alpha,
					 float v_beta,
					 float i_alpha,
					 float i_beta,
					 float dt,
...
static void pll_run(float phase,
					float dt,
...
static void svm(float alpha,
				float beta,
				uint32_t PWMHalfPeriod,
				uint32_t* tAout,
				uint32_t* tBout,
				uint32_t* tCout,
				uint32_t* svm_sector);
static void run_pid_control_pos(float angle_now,
								float angle_set,
								float dt,
								volatile motor_all_state_t* motor);
...
static float correct_encoder(float obs_angle,
							 float enc_angle,
							 float speed,
							 float sl_erpm,
							 volatile motor_all_state_t* motor);
...
#ifdef HW_USE_BRK
	// Enable BRK function. Hardware will asynchronously stop any PWM activity
	// upon an external fault signal. PWM outputs remain disabled until MCU is
	// reset. software will catch the BRK flag to report the fault code
...
void mcpwm_foc_init(volatile mc_configuration* conf_m1,
					volatile mc_configuration* conf_m2)
{
	utils_sys_lock_cnt();
	...
	// Sample intervals. For now they are fixed with voltage samples in the
	// center of V7 and current samples in the center of V0
	TIMER_UPDATE_SAMP(MCPWM_FOC_CURRENT_SAMP_OFFSET);
	...
	chThdCreateStatic(timer_thread_wa,
					  sizeof(timer_thread_wa),
					  NORMALPRIO,
					  timer_thread,
					  NULL);
	...
	terminal_register_command_callback("foc_plot_hfi_en",
									   "Enable HFI plotting. 0: off, 1: DFT, "
									   "2: Raw",
									   "[en]",
									   terminal_plot_hfi);
...
void mcpwm_foc_deinit(void)
{
	...
	timer_thd_stop = true;
	while (timer_thd_stop) { chThdSleepMilliseconds(1); }

	hfi_thd_stop = true;
	while (hfi_thd_stop) { chThdSleepMilliseconds(1); }
...
bool mcpwm_foc_init_done(void) { return m_init_done; }

void mcpwm_foc_set_configuration(volatile mc_configuration* configuration)
{
	motor_now()->m_conf = configuration;

	// Below we check if anything in the configuration changed that requires
	// stopping the motor.
...
mc_state mcpwm_foc_get_state(void) { return motor_now()->m_state; }

bool mcpwm_foc_is_dccal_done(void) { return m_dccal_done; }
 ...
 * 2: Motor 2
 */
int mcpwm_foc_isr_motor(void) { return m_isr_motor; }
 ...
 * stop the motor.
 *
 * WARNING: This function does not use ramping. A too large step with a large
 * motor can destroy hardware.
 *
 * @param dutyCycle
 * The duty cycle to use.
 */
...
void mcpwm_foc_set_openloop(float current, float rpm)
{
	...
	utils_truncate_number(&current,
						  -motor_now()->m_conf->l_current_max *
							  motor_now()->m_conf->l_current_max_scale,
						  motor_now()->m_conf->l_current_max *
							  motor_now()->m_conf->l_current_max_scale);
...
float mcpwm_foc_get_pid_pos_set(void) { return motor_now()->m_pos_pid_set; }

float mcpwm_foc_get_pid_pos_now(void) { return motor_now()->m_pos_pid_now; }
...
bool mcpwm_foc_is_using_encoder(void) { return motor_now()->m_using_encoder; }

float mcpwm_foc_get_tot_current_motor(bool is_second_motor)
{
	volatile motor_all_state_t* motor = M_MOTOR(is_second_motor);
	return SIGN(motor->m_motor_state.vq * motor->m_motor_state.iq) *
		   motor->m_motor_state.i_abs;
}
 ...
 * The D axis current.
 */
float mcpwm_foc_get_id(void) { return motor_now()->m_motor_state.id; }
 ...
 * The Q axis current.
 */
float mcpwm_foc_get_iq(void) { return motor_now()->m_motor_state.iq; }
 ...
 *
 * @return
 * The previous tachometer value in motor steps. The number of motor revolutions
 * will be this number divided by (3 * MOTOR_POLE_NUMBER).
 */
...
float mcpwm_foc_get_vd(void) { return motor_now()->m_motor_state.vd; }

float mcpwm_foc_get_vq(void) { return motor_now()->m_motor_state.vq; }
 ...
 */
void mcpwm_foc_get_current_offsets(volatile int* curr0_offset,
								   volatile int* curr1_offset,
								   volatile int* curr2_offset,
 ...
 */
void mcpwm_foc_encoder_detect(float current,
							  bool print,
							  float* offset,
							  float* ratio,
							  bool* inverted)
{
	...
	for (int i = 0; i < it_rat; i++)
	{
		...
		if (i > 3 && fabsf(utils_angle_difference_rad(motor->m_phase_now_encoder,
	...
	*ratio	   = roundf(((2.0 / 3.0) * 180.0) / fabsf(diff));
	...
	// Rotate
	for (float i = motor->m_phase_now_override; i < 2.0 * M_PI;
		 i += (2.0 * M_PI) / 500.0)
	{
		motor->m_phase_now_override = i;
	...
	{
		float step = (2.0 * M_PI * motor->m_conf->foc_encoder_ratio) /
					 ((float)it_ofs);
		float override = (float)i * step;

		while (motor->m_phase_now_override != override)
		{
			utils_step_towards((float*)&motor->m_phase_now_override,
							   override,
							   step / 100.0);
...
float mcpwm_foc_measure_inductance(float duty, int samples, float* curr, float* ld_lq_diff)
{
	...
	for (int i = 0; i < (samples / 10); i++)
	{
		...
		motor->m_hfi.fft_bin0_func((float*)motor->m_hfi.buffer_current,
								   &real_bin0_i,
								   &imag_bin0_i);
...
/**
 * Measure the motor inductance with short voltage pulses. The difference from
 * the other function is that this one will aim for a specific measurement
 * current. It will also use an appropriate switching frequency.
 *
 * @param curr_goal
 ...
 * The average d and q axis inductance in uH.
 */
float mcpwm_foc_measure_inductance_current(float curr_goal,
										   int samples,
										   float* curr,
										   float* ld_lq_diff)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 360; j++)
		{
			...
			int hall = utils_read_hall(motor != &m_motor_1,
									   motor->m_conf->m_hall_extra_samples);
			float s, c;
...
void mcpwm_foc_print_state(void)
{
	...
	commands_printf("id_filter:    %.2f",
					(double)motor_now()->m_motor_state.id_filter);
	commands_printf("iq_filter:    %.2f",
					(double)motor_now()->m_motor_state.iq_filter);
	commands_printf("id_target:    %.2f",
					(double)motor_now()->m_motor_state.id_target);
	commands_printf("iq_target:    %.2f",
					(double)motor_now()->m_motor_state.iq_target);
	commands_printf("i_abs:        %.2f", (double)motor_now()->m_motor_state.i_abs);
	commands_printf("i_abs_filter: %.2f",
					(double)motor_now()->m_motor_state.i_abs_filter);
...
void mcpwm_foc_tim_sample_int_handler(void)
{
	if (m_init_done)
	{
		...
		virtual_motor_int_handler(m_motor_1.m_motor_state.v_alpha,
...
void mcpwm_foc_adc_int_handler(void* p, uint32_t flags)
{
	...
	volatile motor_all_state_t* motor_other = is_second_motor ? &m_motor_1 :
																  &m_motor_2;
	...
	if (motor_other->m_duty_next_set)
	{
		...
		if (is_second_motor)
		{
			TIMER_UPDATE_DUTY_M1(motor_other->m_duty1_next,
								 motor_other->m_duty2_next,
								 motor_other->m_duty3_next);
		}
		else
		{
			TIMER_UPDATE_DUTY_M2(motor_other->m_duty1_next,
								 motor_other->m_duty2_next,
								 motor_other->m_duty3_next);
		}
#else
		TIMER_UPDATE_DUTY_M1(motor_now->m_duty1_next,
							 motor_now->m_duty2_next,
							 motor_now->m_duty3_next);
#ifdef HW_HAS_DUAL_PARALLEL
		TIMER_UPDATE_DUTY_M2(motor_now->m_duty1_next,
							 motor_now->m_duty2_next,
							 motor_now->m_duty3_next);
#endif
#endif
	...
	ADC_curr_norm_value[2 + norm_curr_ofs] =
		-(ADC_curr_norm_value[0] + ADC_curr_norm_value[1]);
#endif
	...
	{
		// High current sampling mode. Choose the lower currents to derive the
		// highest one in order to be able to measure higher currents.
	...
	else
	{
		...
		if (is_v7)
		{
			...
			{
				// Use the same 2 shunts on low modulation, as that will avoid
				// jumps in the current reading. This is especially important
				// when using HFI.
				ADC_curr_norm_value[2 + norm_curr_ofs] =
					-(ADC_curr_norm_value[0 + norm_curr_ofs] +
					  ADC_curr_norm_value[1 + norm_curr_ofs]);
		...
		if (tim->CCR1 < (tim->ARR - 500) && tim->CCR2 < (tim->ARR - 500))
		{
			// Use the same 2 shunts on low modulation, as that will avoid jumps
			// in the current reading. This is especially important when using HFI.
			ADC_curr_norm_value[2 + norm_curr_ofs] =
				-(ADC_curr_norm_value[0 + norm_curr_ofs] +
				  ADC_curr_norm_value[1 + norm_curr_ofs]);
		}
	...
	// This has to be done for the skip function to have any chance at working
	// with the observer and control loops.
	// TODO: Test this.
	...
	const float phase_diff = utils_angle_difference_rad(motor_now->m_motor_state.phase,
														motor_now->m_phase_before);
	...
	if (motor_now->m_state == MC_STATE_RUNNING)
	{
		...
		//		m_motor_state.i_alpha = (2.0 / 3.0) * ia - (1.0 / 3.0) * ib - (1.0
		/// 3.0) * ic; 		m_motor_state.i_beta = ONE_BY_SQRT3 * ib - ONE_BY_SQRT3 * ic;
		...
		UTILS_LP_FAST(motor_now->m_duty_filtered,
		...
		if (motor_now->m_control_mode == CONTROL_MODE_CURRENT_BRAKE &&
			...
			(motor_now->m_motor_state.i_abs *
			 (motor_now->m_was_full_brake ? 1.0 : 1.5)) < fabsf(motor_now->m_iq_set))
		{
			control_duty				= true;
			duty_set					= 0.0;
		...
		// Reset integrator when leaving duty cycle mode, as the windup
		// protection is not too fast. Making better windup protection is
		// probably better, but not easy.
		if (!control_duty && motor_now->m_was_control_duty)
		{
				...
				conf_now->foc_cc_decoupling == FOC_CC_DECOUPLING_CROSS_BEMF)
			{
				motor_now->m_motor_state.vq_int -=
					motor_now->m_motor_state.speed_rad_s *
		...
		if (control_duty)
		{
			...
			{
				// Truncating the duty cycle here would be dangerous, so run a
				// PID controller.
				...
				motor_now->m_duty_i_term +=
					error * (conf_now->foc_duty_dowmramp_ki * dt) * scale;
			...
			{
				// If the duty cycle is less than or equal to the set duty cycle
				// just limit the modulation and use the maximum allowed
				// current.
		...
		{
			observer_update(motor_now->m_motor_state.v_alpha,
							motor_now->m_motor_state.v_beta,
							motor_now->m_motor_state.i_alpha,
							motor_now->m_motor_state.i_beta,
							dt,
							&motor_now->m_observer_x1,
							&motor_now->m_observer_x2,
		...
		case FOC_SENSOR_MODE_ENCODER:
			if (encoder_index_found() || virtual_motor_is_connected())
			{
				motor_now->m_motor_state.phase =
					correct_encoder(motor_now->m_phase_now_observer,
		...
		case FOC_SENSOR_MODE_HFI:
			if (fabsf(motor_now->m_speed_est_fast * (60.0 / (2.0 * M_PI))) >
		...
		{
			// Force the phase to 0 in handbrake mode so that the current simply
			// locks the rotor.
		...
		if (ld_lq_diff != 0.0)
		{
			...
			id_set_tmp = (lambda - sqrtf(SQ(lambda) + 8.0 * SQ(ld_lq_diff) *
														  SQ(iq_set_tmp))) /
						 (4.0 * ld_lq_diff);
		...
		utils_saturate_vector_2d(&id_set_tmp,
								 &iq_set_tmp,
								 utils_max_abs(conf_now->lo_current_max,
											   conf_now->lo_current_min));
	...
	else
	{
		...
		motor_now->m_motor_state.v_alpha = (2.0 / 3.0) * Va - (1.0 / 3.0) * Vb -
										   (1.0 / 3.0) * Vc;
		motor_now->m_motor_state.v_beta = ONE_BY_SQRT3 * Vb - ONE_BY_SQRT3 * Vc;

#ifdef HW_USE_LINE_TO_LINE
		// rotate alpha-beta 30 degrees to compensate for line-to-line phase
		// voltage sensing
		float x_tmp = motor_now->m_motor_state.v_alpha;
		float y_tmp = motor_now->m_motor_state.v_beta;

		motor_now->m_motor_state.v_alpha = x_tmp * COS_MINUS_30_DEG -
										   y_tmp * SIN_MINUS_30_DEG;
		motor_now->m_motor_state.v_beta = x_tmp * SIN_MINUS_30_DEG +
										  y_tmp * COS_MINUS_30_DEG;
		...
		case FOC_SENSOR_MODE_HFI: {
			motor_now->m_motor_state.phase = motor_now->m_phase_now_observer;
			if (fabsf(motor_now->m_pll_speed * (60.0 / (2.0 * M_PI))) <
		...
		// Park transform
		float vd_tmp = c * motor_now->m_motor_state.v_alpha +
					   s * motor_now->m_motor_state.v_beta;
		float vq_tmp = c * motor_now->m_motor_state.v_beta -
					   s * motor_now->m_motor_state.v_alpha;
		...
		if (conf_now->foc_cc_decoupling == FOC_CC_DECOUPLING_BEMF ||
			conf_now->foc_cc_decoupling == FOC_CC_DECOUPLING_CROSS_BEMF)
		{
			motor_now->m_motor_state.vq_int -= motor_now->m_motor_state.speed_rad_s *
											   conf_now->foc_motor_flux_linkage;
		}

		// Update corresponding modulation
		motor_now->m_motor_state.mod_d =
			motor_now->m_motor_state.vd /
			((2.0 / 3.0) * motor_now->m_motor_state.v_bus);
		motor_now->m_motor_state.mod_q =
			motor_now->m_motor_state.vq /
			((2.0 / 3.0) * motor_now->m_motor_state.v_bus);
	}

	// Calculate duty cycle
	motor_now->m_motor_state.duty_now = SIGN(motor_now->m_motor_state.vq) *
										sqrtf(SQ(motor_now->m_motor_state.mod_d) +
											  SQ(motor_now->m_motor_state.mod_q)) /
										SQRT3_BY_2;

	// Run PLL for speed estimation
	pll_run(motor_now->m_motor_state.phase,
			dt,
			&motor_now->m_pll_phase,
	...
	{
		// Based on back emf and motor parameters. This could be useful for a
		// resistance observer in the future.
		//		UTILS_LP_FAST(m_speed_est_fast, (m_motor_state.vq - (3.0 / 2.0)
		//* m_conf->foc_motor_r * m_motor_state.iq) /
		// m_conf->foc_motor_flux_linkage, 0.05);
	...
	if (motor_now->m_state == MC_STATE_RUNNING)
	{
		run_pid_control_pos(motor_now->m_pos_pid_now,
							motor_now->m_pos_pid_set,
							dt,
							motor_now);
	}
...
static void timer_update(volatile motor_all_state_t* motor, float dt)
{
	...
	float openloop_rpm_max = utils_map(fabsf(motor->m_motor_state.iq_filter),
									   0.0,
									   motor->m_conf->l_current_max,
									   motor->m_conf->foc_openloop_rpm_low *
	...
	if (motor->m_conf->foc_sensor_mode != FOC_SENSOR_MODE_ENCODER)
	{
		...
		{
			openloop_rpm =
				utils_map(time_fwd, t_lock, t_lock + t_ramp, 0.0, openloop_rpm);
		}
	}
	...
	if (motor->m_control_mode == CONTROL_MODE_CURRENT_BRAKE ||
		(motor->m_state == MC_STATE_RUNNING &&
		 fabsf(motor->m_motor_state.duty_now) < 0.001))
	{
	...
	if (motor->m_min_rpm_timer > 0.0)
	{
		...
		// When the motor gets stuck it tends to be 90 degrees off, so start the
		// open loop sequence by correcting with 90 degrees.
		if (started_now)
		{
			if (motor->m_motor_state.duty_now > 0.0)
			{
	...
	// 0.0, 1.0, 			motor->m_conf->foc_observer_gain *
	...
	float gamma_tmp = utils_map(fabsf(motor->m_motor_state.duty_now),
								0.0,
								40.0 / motor->m_motor_state.v_bus,
								0,
								motor->m_conf->foc_observer_gain);
	if (gamma_tmp <
		(motor->m_conf->foc_observer_gain_slow * motor->m_conf->foc_observer_gain))
	{
		gamma_tmp = motor->m_conf->foc_observer_gain_slow *
					motor->m_conf->foc_observer_gain;
	}

	// 4.0 scaling is kind of arbitrary, but it should make configs from old
	// VESC Tools more likely to work.
...
static void hfi_update(volatile motor_all_state_t* motor)
{
	...
	if (motor->m_hfi.ready)
	{
		...
		// Assuming this thread is much faster than it takes to fill the HFI
		// buffer completely, we should lag 1/2 HFI buffer behind in phase.
		...
		angle_bin_2 += motor->m_motor_state.speed_rad_s *
					   ((float)motor->m_hfi.samples / 2.0) * dt_sw;
		...
		if (motor->m_hfi.est_done_cnt < motor->m_conf->foc_hfi_start_samples)
		{
			...
			if (fabsf(utils_angle_difference_rad(angle_bin_2, angle_bin_1)) >
				(M_PI / 2.0))
			{
		...
		// As angle_bin_1 is based on saturation, it is only accurate when the
		// motor current is low. It might be possible to compensate for that,
		// which would allow HFI on non-salient motors.
		...
		if (motor->m_hfi_plot_en == 1)
		{
			...
			if (hfi_plot_div >= 8)
			{
				...
				float real_bin0, imag_bin0;
				motor->m_hfi.fft_bin0_func((float*)motor->m_hfi.buffer,
										   &real_bin0,
										   &imag_bin0);
					 ...
					 volatile motor_all_state_t* motor)
{
	...
	const float sign =
		(motor->m_motor_state.iq * motor->m_motor_state.vq) >= 0.0 ? 1.0 : -1.0;
	R -= R * sign * conf_now->foc_sat_comp *
...
static void control_current(volatile motor_all_state_t* motor, float dt)
{
	...
	// get sin & cos value from motor phase
	float c, s;
	...
	// Test if do hfi
	bool do_hfi =
		conf_now->foc_sensor_mode == FOC_SENSOR_MODE_HFI &&
		!motor->m_phase_override &&
		abs_rpm < (conf_now->foc_sl_erpm_hfi * (motor->m_cc_was_hfi ? 1.8 : 1.5));
	motor->m_cc_was_hfi = do_hfi;

	// Only allow Q axis current after the HFI ambiguity is resolved. This
	// causes a short delay when starting.
	...
	// Park Transform, from alpha beta to dq
	...
	{
		d_gain_scale = utils_map(max_mod_norm,
								 conf_now->foc_d_gain_scale_start,
								 1.0,
								 1.0,
		...
		conf_now->foc_cc_decoupling != FOC_CC_DECOUPLING_DISABLED)
	{
		...
		case FOC_CC_DECOUPLING_CROSS:
			dec_vd = state_m->iq * state_m->speed_rad_s *
					 conf_now->foc_motor_l * (3.0 / 2.0);
			dec_vq = state_m->id * state_m->speed_rad_s *
					 conf_now->foc_motor_l * (3.0 / 2.0);
			break;
	...
	// Currently we start by truncating the d-axis and then the q-axis with the
	// magnitude that is left. Axis decoupling is taken into account in the
	// truncation. How to do that best is also an open question...

	// Take both cross and back emf decoupling into consideration. Seems to make
	// the control noisy at full modulation.
	//	utils_truncate_number((float*)&state_m->vd_int, -max_v_mag + dec_vd,
	// max_v_mag + dec_vd); 	float mag_left = sqrtf(SQ(max_v_mag) -
	// SQ(state_m->vd_int - dec_vd)); utils_truncate_number((float*)&state_m->vq_int,
	//-mag_left - (dec_vq + dec_bemf), mag_left - (dec_vq + dec_bemf));
	...
	utils_truncate_number((float*)&state_m->vq_int,
						  -mag_left - dec_bemf,
						  mag_left - dec_bemf);

	// Ignore decoupling. Works badly when back emf decoupling is used, probably
	// not the best way to go.
	//	utils_truncate_number((float*)&state_m->vd_int, -max_v_mag, max_v_mag);
	//	float mag_left = sqrtf(SQ(max_v_mag) - SQ(state_m->vd_int));
	//	utils_truncate_number((float*)&state_m->vq_int, -mag_left, mag_left);

	// This is how anti-windup was done in FW < 4.0. Does not work well when
	// there is too much D axis voltage.
	//	utils_truncate_number((float*)&state_m->vd_int, -max_v_mag, max_v_mag);
	...
	const float mod_alpha_filter_sgn = (2.0 / 3.0) * SIGN(ia_filter) -
									   (1.0 / 3.0) * SIGN(ib_filter) -
									   (1.0 / 3.0) * SIGN(ic_filter);
	...
	if (do_hfi)
	{
		...
		{
			hfi_voltage = utils_map(fabsf(state_m->iq),
									0.0,
									conf_now->l_current_max,
									conf_now->foc_hfi_voltage_run,
									conf_now->foc_hfi_voltage_max);
		}
		...
		if (motor->m_hfi.is_samp_n)
		{
			...
			{
				motor->m_hfi.buffer[motor->m_hfi.ind] =
					((hfi_voltage / 2.0 - conf_now->foc_motor_r * current_sample) /
					 (conf_now->foc_f_sw * current_sample));
			}
		...
		{
			// Delay adding the HFI voltage when not sampling in both 0 vectors,
			// as it will cancel itself with the opposite pulse from the previous
			// HFI sample. This makes more sense when drawing the SVM waveform.
			svm(-mod_alpha_tmp,
				-mod_beta_tmp,
				TIM1->ARR,
								...
								volatile motor_all_state_t* motor)
{
	...
	{
		d_term = (error - motor->m_pos_prev_error) *
				 (conf_now->p_pid_kd / motor->m_pos_dt_int);
...
static void run_pid_control_speed(float dt, volatile motor_all_state_t* motor)
{
	...
	d_term = (error - motor->m_speed_prev_error) * (conf_now->s_pid_kd / dt) *
			 (1.0 / 20.0);
...
static float correct_hall(float angle, float dt, volatile motor_all_state_t* motor)
{
	...
	int ang_hall_int =
		conf_now->foc_hall_table[utils_read_hall(motor != &m_motor_1, conf_now->m_hall_extra_samples)];
	...
	if (ang_hall_int < 201)
	{
		...
		else if (ang_hall_int != motor->m_ang_hall_int_prev)
		{
			...
			// A transition was just made. The angle is in the middle of the new
			// and old angle.
		...
		if (((60.0 / (2.0 * M_PI)) *
			 ((M_PI / 3.0) / fmaxf(fabsf(motor->m_hall_dt_diff_now),
								   fabsf(motor->m_hall_dt_diff_last)))) <
			conf_now->foc_hall_interp_erpm)
		{
			// Don't interpolate on very low speed, just use the closest hall
			// sensor. The reason is that we might get stuck at 60 degrees off
			// if a direction change happens between two steps.
			motor->m_ang_hall = ang_hall_now;
		}
		else
		{
...
static void terminal_plot_hfi(int argc, const char** argv)
{
	if (argc == 2)
	{
		...
		if (d == 0 || d == 1 || d == 2)
		{
			...
			commands_printf(motor_now()->m_hfi_plot_en ? "HFI plot enabled" :
														   "HFI plot disabled");
		}
		else
		{
