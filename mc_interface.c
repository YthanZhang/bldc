...
__attribute__((
	section(".ram4"))) static volatile int16_t m_curr0_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_curr1_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_ph1_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_ph2_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_ph3_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_vzero_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((section(
	".ram4"))) static volatile uint8_t m_status_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((section(
	".ram4"))) static volatile int16_t m_curr_fir_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int16_t m_f_sw_samples[ADC_SAMPLE_MAX_LEN];
__attribute__((
	section(".ram4"))) static volatile int8_t m_phase_samples[ADC_SAMPLE_MAX_LEN];
...
void mc_interface_init(void)
{
	...
	chThdCreateStatic(timer_thread_wa,
		...
		timer_thread,
		NULL);
	chThdCreateStatic(sample_send_thread_wa,
		sizeof(sample_send_thread_wa),
		NORMALPRIO - 1,
		sample_send_thread,
		NULL);
	chThdCreateStatic(fault_stop_thread_wa,
		sizeof(fault_stop_thread_wa),
		HIGHPRIO - 3,
		fault_stop_thread,
		NULL);
	...
	case SENSOR_PORT_MODE_SINCOS:
		encoder_init_sincos(motor_now()->m_conf.foc_encoder_sin_gain,
			motor_now()->m_conf.foc_encoder_sin_offset,
			motor_now()->m_conf.foc_encoder_cos_gain,
			motor_now()->m_conf.foc_encoder_cos_offset,
...
void mc_interface_set_configuration(mc_configuration* configuration)
{
	...
	if (motor->m_conf.m_sensor_port_mode != configuration->m_sensor_port_mode)
	{
		...
		case SENSOR_PORT_MODE_SINCOS:
			encoder_init_sincos(motor->m_conf.foc_encoder_sin_gain,
				motor->m_conf.foc_encoder_sin_offset,
				motor->m_conf.foc_encoder_cos_gain,
				motor->m_conf.foc_encoder_cos_offset,
 ...
 */
void mc_interface_lock(void) { motor_now()->m_lock_enabled = true; }

/**
 * Unlock all control commands.
 */
void mc_interface_unlock(void) { motor_now()->m_lock_enabled = false; }
...
mc_fault_code mc_interface_get_fault(void) { return motor_now()->m_fault_now; }
...
void mc_interface_set_brake_current_rel(float val)
{
	...
	mc_interface_set_brake_current(
		val * fabsf(motor_now()->m_conf.lo_current_motor_min_now));
}
 ...
 */
void mc_interface_release_motor(void) { mc_interface_set_current(0.0f); }
...
int mc_interface_set_tachometer_value(int steps)
{
	...
	case MOTOR_TYPE_BLDC:
	case MOTOR_TYPE_DC:
		ret = mcpwm_set_tachometer_value(DIR_MULT * (float)steps);
		break;

	case MOTOR_TYPE_FOC:
		ret = mcpwm_foc_set_tachometer_value(DIR_MULT * (float)steps);
		break;
...
float mc_interface_read_reset_avg_motor_current(void)
{
	...
	float res = motor_now()->m_motor_current_sum /
				motor_now()->m_motor_current_iterations;
...
float mc_interface_read_reset_avg_input_current(void)
{
	...
	float res = motor_now()->m_input_current_sum /
				motor_now()->m_input_current_iterations;
...
float mc_interface_get_pid_pos_set(void) { return motor_now()->m_position_set; }
 ...
 */
float mc_interface_temp_fet_filtered(void) { return motor_now()->m_temp_fet; }
...
/**
 * Get the battery level, based on battery settings in configuration. Notice
 * that this function is based on remaining watt hours, and not amp hours.
 *
 * @param wh_left
...
float mc_interface_get_battery_level(float* wh_left)
{
	...
	case BATTERY_TYPE_LIION_3_0__4_2:
		battery_avg_voltage = ((3.2f + 4.2f) / 2.0f) *
							  (float)(conf->si_battery_cells);
		battery_avg_voltage_left =
			((3.2f * (float)(conf->si_battery_cells) + v_in) / 2.0f);
		float batt_left =
			utils_map(v_in / (float)(conf->si_battery_cells), 3.2f, 4.2f, 0.0f, 1.0f);
		batt_left = utils_batt_lion_norm_v_to_capacity(batt_left);
		ah_tot *= 0.85f;	// 0.85 because the battery is not fully depleted
							// at 3.2V / cell
		ah_left = batt_left * ah_tot;
		break;

	case BATTERY_TYPE_LIIRON_2_6__3_6:
		battery_avg_voltage = ((2.8f + 3.6f) / 2.0f) *
							  (float)(conf->si_battery_cells);
		battery_avg_voltage_left =
			((2.8f * (float)(conf->si_battery_cells) + v_in) / 2.0f);
	...
	case BATTERY_TYPE_LEAD_ACID:
		...
		battery_avg_voltage = ((2.1f + 2.36f) / 2.0f) *
							  (float)(conf->si_battery_cells);
		battery_avg_voltage_left =
			((2.1f * (float)(conf->si_battery_cells) + v_in) / 2.0f);
		ah_left = utils_map(v_in / (float)(conf->si_battery_cells),
			2.1f,
			2.36f,
			0.0f,
...
{
	const volatile mc_configuration* conf = mc_interface_get_configuration();
	const float rpm = mc_interface_get_rpm() / ((float)conf->si_motor_poles / 2.0f);
	return (rpm / 60.0f) * conf->si_wheel_diameter * (float)M_PI / conf->si_gear_ratio;
}

/**
 * Get the distance traveled based on wheel diameter, gearing and motor pole
 * settings.
 *
 * @return
...
float mc_interface_get_distance(void)
{
	...
	const float tacho_scale =
		(conf->si_wheel_diameter * (float)M_PI) /
		(3.0f * (float)conf->si_motor_poles * conf->si_gear_ratio);

	return (float)mc_interface_get_tachometer_value(false) * tacho_scale;
}

/**
 * Get the absolute distance traveled based on wheel diameter, gearing and motor
 * pole settings.
 *
 * @return
...
float mc_interface_get_distance_abs(void)
{
	...
	const float tacho_scale =
		(conf->si_wheel_diameter * (float)M_PI) /
		(3.0f * (float)conf->si_motor_poles * conf->si_gear_ratio);
	return (float)mc_interface_get_tachometer_abs_value(false) * tacho_scale;
}
...
void mc_interface_mc_timer_isr(bool is_second_motor)
{
	...
	if (input_voltage < conf_now->l_min_vin || input_voltage > conf_now->l_max_vin)
	{
	...
	// Fetch these values in a config-specific way to avoid some overhead of the
	// general functions. That will make this interrupt run a bit faster.
	mc_state state;
	float current;
	...
	if (conf_now->motor_type == MOTOR_TYPE_FOC)
	{
		...
		current_in_filtered = mcpwm_foc_get_tot_current_in_filtered_motor(
			is_second_motor);
		abs_current = mcpwm_foc_get_abs_motor_current_motor(is_second_motor);
		abs_current_filtered = mcpwm_foc_get_abs_motor_current_filtered_motor(
			is_second_motor);
	}
	else
	{
	...
	bool sample						= false;
	debug_sampling_mode sample_mode = m_sample_is_second_motor == is_second_motor ?
											m_sample_mode :
											DEBUG_SAMPLING_OFF;
	...
	if (sample)
	{
		...
		if (a >= m_sample_int)
		{
			...
			if (conf_now->motor_type == MOTOR_TYPE_FOC)
			{
				...
				m_phase_samples[m_sample_now] = (uint8_t)(
					mcpwm_foc_get_phase() / 360.0f * 250.0f);
				//				m_phase_samples[m_sample_now] =
				//(uint8_t)(mcpwm_foc_get_phase_observer() / 360.0 * 250.0);
				// float ang =
				// utils_angle_difference(mcpwm_foc_get_phase_observer(),
				// mcpwm_foc_get_phase_encoder()) + 180.0;
			...
			m_curr_fir_samples[m_sample_now] = (int16_t)(
				current * (8.0 / FAC_CURRENT));
			m_f_sw_samples[m_sample_now]   = (int16_t)(f_samp / 10.0);
			m_status_samples[m_sample_now] = mcpwm_get_comm_step() |
											 (mcpwm_read_hall_phase() << 3);
...
/**
 * Update the override limits for a configuration based on MOSFET temperature
 * etc.
 *
 * @param conf
	...
	volatile mc_configuration* conf)
{
	...
	UTILS_LP_FAST(motor->m_temp_fet,
		NTC_TEMP(is_motor_1 ? ADC_IND_TEMP_MOS : ADC_IND_TEMP_MOS_M2),
		0.1);
	...
	case TEMP_SENSOR_PTC_1K_100C:
		temp_motor = is_motor_1 ?
						   PTC_TEMP_MOTOR(1000.0, conf->m_ptc_motor_coeff, 100) :
						   PTC_TEMP_MOTOR_2(1000.0, conf->m_ptc_motor_coeff, 100);
		break;

	case TEMP_SENSOR_KTY83_122: {
		// KTY83_122 datasheet used to approximate resistance at given
		// temperature to cubic polynom
		...
		// You can change pull up resistor and update NTC_RES_MOTOR for your
		// hardware without changing polynom
		float res = NTC_RES_MOTOR(
			ADC_Value[is_motor_1 ? ADC_IND_TEMP_MOTOR : ADC_IND_TEMP_MOTOR_2]);
		float pow2 = res * res;
		temp_motor = 0.0000000102114874947423 * pow2 * res -
					 0.000069967997703501 * pow2 + 0.243402040973194 * res -
					 160.145048329356;
	}
	break;
	}

	// If the reading is messed up (by e.g. reading 0 on the ADC and dividing by
	// 0) we avoid putting an invalid value in the filter, as it will never
	// recover. It is probably safest to keep running the motor even if the
	// temperature reading fails. A config option to reduce power on invalid
	// temperature readings might be useful.
	...
	else
	{
		...
		maxc = utils_map(motor->m_temp_fet,
	...
	else
	{
		...
		maxc = utils_map(motor->m_temp_motor,
	...
	{
		lo_fet_temp_accel = utils_map(motor->m_temp_fet,
	...
	{
		lo_motor_temp_accel = utils_map(motor->m_temp_motor,
	...
	{
		lo_max_rpm =
			utils_map(rpm_now, rpm_pos_cut_start, rpm_pos_cut_end, l_current_max_tmp, 0.0);
	}
	...
	{
		lo_min_rpm =
			utils_map(rpm_now, rpm_neg_cut_start, rpm_neg_cut_end, l_current_max_tmp, 0.0);
	}
	...
	//
	//	if (duty_abs > 0.001) {
	//		conf->lo_current_motor_max_now =
	// utils_min_abs(conf->lo_current_max, conf->lo_in_current_max / duty_abs);
	// conf->lo_current_motor_min_now = utils_min_abs(conf->lo_current_min,
	// conf->lo_in_current_min / duty_abs); 	} else {
	// conf->lo_current_motor_max_now = conf->lo_current_max;
	//		conf->lo_current_motor_min_now = conf->lo_current_min;
	//	}

	// Note: The above code should work, but many people have reported issues
	// with it. Leaving it disabled for now until I have done more
	// investigation.
	conf->lo_current_motor_max_now = conf->lo_current_max;
	conf->lo_current_motor_min_now = conf->lo_current_min;
}
...
static void run_timer_tasks(volatile motor_if_state_t* motor)
{
	...
	if (motor->m_conf.motor_type == MOTOR_TYPE_FOC &&
		...
		mcpwm_foc_is_using_encoder() && encoder_spi_get_error_rate() > 0.05)
	{
	...
	if (motor->m_conf.motor_type == MOTOR_TYPE_FOC &&
		motor->m_conf.foc_sensor_mode == FOC_SENSOR_MODE_ENCODER &&
		mcpwm_foc_is_using_encoder() && encoder_get_no_magnet_error_rate() > 0.05)
	{
...
#ifdef HW_HAS_DUAL_MOTORS
		mcpwm_foc_get_current_offsets(&curr0_offset,
			&curr1_offset,
			&curr2_offset,
			motor == &m_motor_2);
#else
...
#define MIDDLE_ADC 2048
	...
	{	 // This won't work when high
		 // current sampling is used
		motor->m_motor_current_unbalance =
			mc_interface_get_abs_motor_current_unbalance();

		if (motor->m_motor_current_unbalance > MCCONF_MAX_CURRENT_UNBALANCE)
		{
			UTILS_LP_FAST(motor->m_motor_current_unbalance_error_rate,
				1.0,
				(1 / 1000.0));
		}
		else
		...
		if (motor->m_motor_current_unbalance_error_rate >
...
static THD_FUNCTION(sample_send_thread, arg)
{
	...
	for (;;)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			while (ind_samp < 0) { ind_samp += ADC_SAMPLE_MAX_LEN; }

			buffer[index++] = COMM_SAMPLE_PRINT;
			buffer_append_float32_auto(buffer,
				(float)m_curr0_samples[ind_samp] * FAC_CURRENT,
				&index);
			buffer_append_float32_auto(buffer,
				(float)m_curr1_samples[ind_samp] * FAC_CURRENT,
				&index);
			buffer_append_float32_auto(buffer,
			...
			buffer_append_float32_auto(buffer,
				...
				&index);
			buffer_append_float32_auto(buffer,
				(float)m_f_sw_samples[ind_samp] * 10.0,
				&index);
...
static THD_FUNCTION(fault_stop_thread, arg)
{
	...
	for (;;)
	{
		...
		volatile motor_if_state_t* motor =
			m_fault_stop_is_second_motor ? &m_motor_2 : &m_motor_1;
#else
		volatile motor_if_state_t* motor = &m_motor_1;
#endif
		...
		{
			// Sent to terminal fault logger so that all faults and their
			// conditions can be printed for debugging.
			utils_sys_lock_cnt();
...
{
	m_odometer_meters = new_odometer_meters -
						roundf(mc_interface_get_distance_abs());
}
